...
export default function middleware(req, res)
{
	...
	for(let name of names)
	{
		...
		if (resource && resource.type && resource.type == 'menubars')
		{
			selection[name] = resource
		}
	}
	
	let html_content = render_node(selection, 1, 'menubars')
	
	
	const html = html_tree_template(html_content, 'Devapt Devtools - Store / Config / Menubars')
