...
function(Devapt, DevaptTrace, DevaptTypes, DevaptOptions, DevaptClasses, DevaptView, undefined)
{
	...
	function DevaptPagination(arg_name, arg_container_jqo, arg_options)
	{
		...
		self.update_pagination_current = function(arg_current_page)
		{
			...
			else
			{
				...
				if (loop_first_page > prefix_last_page + 1)
				{
				...
				self.value(context, 'loop_page', loop_page);
				if (loop_page < suffix_first_page - 1)
				{
		 ...
		 * @memberof			DevaptPagination
		 * @desc				Get page label
		 * @param {string}		page value
		 * @return {string}		page label
		 */
		self.get_page_label = function(arg_page_label)
		{
			var self = this;
			var context = 'get_item_label(page_label)';
			// self.enter(context, '');
			
			var alphabet = 'abcdefghijklmnopqrstuvwxyz';
			var label = DevaptTypes.is_integer(arg_page_label) && arg_page_label > 0 && arg_page_label <= 26 ? alphabet.charAt(arg_page_label - 1) : arg_page_label;
	...
	// DevaptOptions.register_str_option(DevaptPagination, 'pagination_type', 'html', true, []); //  
