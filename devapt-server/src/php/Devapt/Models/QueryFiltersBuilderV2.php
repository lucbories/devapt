...
final class SqlFiltersBuilder
{
	...
	*/
	...
	{
		$context = 'SqlFiltersBuilder.buildFilterNode';
		...
		if ( is_string($filter_modifier) && $filter_modifier !== '' )
		{
			...
			// GET FILTER MODIFIER
			$modifier = SqlFilterOperatorBuilder::buildFilterModifier($filter_type, $filter_modifier, $left_str);
			$left_expr = new Expr($modifier);
			$filter_predicate = SqlFilterOperatorBuilder::buildFilterExprOperator($filter_type, $filter_op, $left_expr, $right_str_array);
		}
		else if ($field_sql_is_expr)
		{
			Trace::step($context, 'filter operator with field expression', self::$TRACE_FILTERS_BUILDER);
			$filter_predicate = SqlFilterOperatorBuilder::buildFilterExprOperator($filter_type, $filter_op, $left_str, $right_str_array);
		}
		else
		{
			Trace::step($context, 'filter operator without modifier near expression', self::$TRACE_FILTERS_BUILDER);
			$filter_predicate = SqlFilterOperatorBuilder::buildFilterOperator($filter_type, $filter_op, $left_str, $right_str_array, $filter_left_type, $filter_right_type);
		}
